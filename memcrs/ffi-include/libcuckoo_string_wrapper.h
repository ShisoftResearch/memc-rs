...
class CuckooStringMap {
  using Table = libcuckoo::cuckoohash_map<UnifiedStr, UnifiedStrLarge, UnifiedStrHash, UnifiedStrEqual>;
  Table table_;
public:
  explicit CuckooStringMap(size_t capacity) : table_(capacity) {}
  bool insert(const UnifiedStr& key, const UnifiedStrLarge& value) { return table_.insert(key, value); }
  bool get(const UnifiedStr& key) { UnifiedStrLarge value; return table_.find(key, value); }
...
bool cuckoo_string_insert_cpp(const std::shared_ptr<CuckooStringMap>& m, UnifiedStr& key, UnifiedStrLarge& value);
bool cuckoo_string_get_cpp(const std::shared_ptr<CuckooStringMap>& m, UnifiedStr& key, UnifiedStrLarge* out_value);
...
bool cuckoo_string_insert(cuckooffi_CuckooStringMapOpaque* map, UnifiedStr& key, UnifiedStrLarge& value);
bool cuckoo_string_get(cuckooffi_CuckooStringMapOpaque* map, UnifiedStr& key, UnifiedStrLarge* out_value);
